...
lm.compare = function(i, x, y){
  ...
  lm.l0l2 = bess(x_train, y_train, type = "bsrr", lambda.max = 100, lambda.min = 0.01)
...
# res = lapply(1:20, lm.compare, x, y)
...
```
Calling the `plot` routine on an `bess` object fitted by `"bsrr"` algorithm will provide a heatmap of GIC of the fitted path. 
```{r}
 #plot(logi.l0l2)
```
